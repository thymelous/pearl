...
class UpdateQueryTest {
  ...
  fun `should produce SQL for individual record updates`() {
    ...
    val (sql, bindings) = updateRecord(changeset).toSql()

    assertEquals("""UPDATE "TestModel" SET "enum" = ?, "name" = ? WHERE "TestModel"."id" = ?""", sql)
  ...
  fun `should produce SQL for predicated updates`() {
    ...
    val (sql, bindings) = updateAll(changeset).where { it["id"] `in` from<TestModel>().where { it["name"] eq "g" }.select("id") }.toSql()
